...
if (dbTms) {
  ...
  dbTms.transaction(function(tx) {
    ...
    tx.executeSql(dbSql, [], null, dbError);
    dbSql = "CREATE TABLE Csbk2_Accept (TrxNo INT,LineItemNo INT, BoxCode TEXT, Status TEXT,Pcs INT,UnitRate TEXT,Volume TEXT,GrossWeight TEXT,CollectedPcs int,CollectedAmt TEXT,DepositAmt TEXT,DiscountAmt TEXT,AttachmentFlag TEXT,ItemNo INT,BookingNo TEXT)";
    tx.executeSql(dbSql, [], null, dbError);
  });
}
...
var db_add_Csbk2_Accept = function(Csbk2) {
  if (dbTms) {
    dbTms.transaction(function(tx) {
      ...
      dbSql = 'INSERT INTO Csbk2_Accept(TrxNo,LineItemNo, BoxCode, Status,Pcs,UnitRate,Volume,GrossWeight,CollectedPcs,CollectedAmt,DepositAmt,DiscountAmt,AttachmentFlag,ItemNo,BookingNo) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
      tx.executeSql(dbSql, [Csbk2.TrxNo,Csbk2.LineItemNo, Csbk2.BoxCode, Csbk2.Status,Csbk2.Pcs,Csbk2.UnitRate,Csbk2.Volume,Csbk2.GrossWeight,Csbk2.CollectedPcs,Csbk2.CollectedAmt,Csbk2.DepositAmt,Csbk2.DiscountAmt,Csbk2.AttachmentFlag,Csbk2.ItemNo,Csbk2.BookingNo], null, dbError);
  });
  }
}
